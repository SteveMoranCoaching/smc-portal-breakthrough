...
export default function CoachActivityFeed({
  ...
  const [typeFilter, setTypeFilter] = useState<"all" | "video" | "log">("all")
          ...
          {filteredItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={`/coach/${item.clientId}?exercise=${encodeURIComponent(
                  item.exercise_name
                )}`}
              ...
              >
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <p
                      className={`mt-1 text-[10px] font-bold uppercase tracking-wide ${
                        isVideo ? "text-yellow-400" : "text-indigo-400"
                      }`}
                    >
                      {isVideo ? "Video" : "Workout Log"}
                    </p>
                  ...
                  <span
                    className={`inline-flex h-[18px] min-w-[58px] items-center justify-center rounded-full border px-2 py-0.5 text-[9px] font-semibold uppercase leading-none tracking-wide ${
                      isNew
                        ? isVideo
                  ...
                  >
                    {isNew ? (isVideo ? "New Video" : "New Log") : "Reviewed"}
                  </span>
                </div>
              </Link>
            )
